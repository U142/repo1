...
public class SendOptionToolbar extends DefaultPanel implements ActionListener, FocusListener {
    ...
    private String eventSourceForFilter;
    
    public String getEventSourceForFilter() {
        return eventSourceForFilter;
    }
    public void setEventSourceForFilter(String eventSourceForFilter) {
        this.eventSourceForFilter = eventSourceForFilter;
    }
    SendObject m_parent;
    ...
    private boolean enablePredefinedFilter = false;
    private JComboCheckBox comboAddressFilterList = null;
    private AddressFilterInfoVO selectedAddressFilter;
    public AddressFilterInfoVO getSelectedAddressFilter() {
        return selectedAddressFilter;
    }
    public void setSelectedAddressFilter(AddressFilterInfoVO selectedAddressFilter) {
        this.selectedAddressFilter = selectedAddressFilter;
    }
   ...
   public void remove_addresstypes(int n_adrtypes) {
    ...
    public void populateABASPanelData(int types, boolean oldPARM)
    {
        ...
        if((SendController.SENDTO_USE_ABAS_RECIPIENTS & types)==SendController.SENDTO_USE_ABAS_RECIPIENTS)
        {
            ...
            comboAddressFilterList.setVisible(chkAddressBased.isSelected());
        }
    ...
    public SendOptionToolbar(SendObject sendobject, ActionListener callback, int n_sendnumber) {
        super();
        isSettings=false;
    ...
    public SendOptionToolbar(SendObject sendobject, ActionListener callback, int n_sendnumber, String eventSourceForFilter) {
        super();
        isSettings=false;
        set_import_callback(this);
        m_parent = sendobject;
    ...
    public SendOptionToolbar(SendObject sendobject, ActionListener callback, int n_sendnumber, boolean fromsetting) {
        super();
        isSettings=fromsetting;
        set_import_callback(this);
        m_parent = sendobject;
        m_n_initsendnumber = n_sendnumber;
        m_callback = callback;
    ...
    public void init() {
        ...
        comboAddressFilterList = new JComboCheckBox();
        log.debug("after creating comboAddressFilterList 1");
        ...
        comboAddressFilterList.setActionCommand("act_set_filters");
	...
	public boolean isSelected(String filterName){
		...
		if(getEventSourceForFilter().equals("newalert")){
		...
		else if(getEventSourceForFilter().equals("predefinedalert")){
			java.util.List<AddressFilterInfoVO>  addressFilterList = Variables.getPredefinedAlertfilterList();
			for(int i=0; i< addressFilterList.size(); i++){
	            if(filterName.equals(addressFilterList.get(i).getFilterName())){
	            PAS.get_pas().get_sendcontroller().getAddressFilters().add(addressFilterList.get(i));
				return true;
			}
			}
			return false;
		}
             if (PAS.get_pas().get_settings().getAddressFilters() != null
                    && PAS.get_pas().get_settings().getAddressFilters()
                            .size() > 0) {
                 java.util.List<AddressFilterInfoVO> addressFilterList  = new ArrayList<AddressFilterInfoVO>();   
             if (PAS.get_pas().get_parmcontroller() != null && PAS.get_pas().get_parmcontroller().getObjectFromTree() != null && PAS.get_pas().get_parmcontroller().getObjectFromTree() instanceof AlertVO){
                 ...
                 addressFilterList = selectedAlert.getFilters();
             } else {
                 addressFilterList = PAS.get_pas().get_settings().getAddressFilters();
             }
                for(int i=0; i< addressFilterList.size(); i++){
    	            if(filterName.equals(addressFilterList.get(i).getFilterName())){
    	            PAS.get_pas().get_sendcontroller().getAddressFilters().add(addressFilterList.get(i));
	 ...
	 private void resetAddressFilterList()
    {
		...
		JCheckBox[] items = new JCheckBox[addressFilterList.size()];
        
        for (int i = 0; i < addressFilterList.size(); i++) {
            items[i] = new JCheckBox(addressFilterList.get(i).getFilterName(), isSelected(addressFilterList.get(i).getFilterName()));
            items[i].setActionCommand(String.valueOf(addressFilterList.get(i).getFilterId()));
         
        }
        comboAddressFilterList = new JComboCheckBox(items);
        ...
        if (getEventSourceForFilter() != null){
            if (getEventSourceForFilter().equals("predefinedalert")) {
                comboAddressFilterList.setSelectedItem(Variables
                        .getPredefinedAlertfilterList());
            } else if (getEventSourceForFilter().equals("newalert")) {
                comboAddressFilterList.setSelectedItem(PAS.get_pas()
                        .get_settings().getAddressFilters());
            } else {
                log.debug("in sendoptiontollbar AddressFilterListCombo size="
                        + comboAddressFilterList.getItemCount());
    ...
    private void setAddressFilter(AddressFilterInfoVO filter){
    ...
    public synchronized void actionPerformed(ActionEvent e) {
        ...
        if("act_open_colorpicker".equals(e.getActionCommand())) {
            ...
            if(e.getSource().equals(m_btn_adrtypes_cell_broadcast_voice))
                ...
                comboAddressFilterList.setVisible(chkAddressBased.isSelected());
                if (m_parent != null) {
                    AddressFilterInfoVO selectedItem = PAS.get_pas().get_sendcontroller().get_activesending().getFilterList().get(0);
                    comboAddressFilterList.setSelectedItem(selectedItem);
                }
            }
            gen_addresstypes();
    ...
    public void lock_sending(boolean b) {
        if(get_parent()!=null && get_parent().setLocked(b)) {
            ...
            if (m_parent != null) {
                comboAddressFilterList.setEnabled(!b);
            }
