 ...
 * @author sachinn
 */
public class PredefinedAreaController extends MainAreaController{
	
	private static final Log log = UmsLog.getLogger(PredefinedAreaController.class);

	private PredefinedAreaPanel predefinedAreaPanel;
	public PredefinedAreaPanel getPredefinedAreaPanel() { return predefinedAreaPanel; }
	...
	public void createGUI() {
		...
		try {
			...
			Error.getError().addError("PredefinedAreaController","IOException in createGUI",e,Error.SEVERITY_ERROR);
		}*/
		initGUI();
	}
...
//		log.debug("inside predefined area controller drawLayers(Graphics g) called");
			...
			if(p!=null) {
				//if(!p.equals(get_shape())){
					try {
						...
						Error.getError().addError("PredefinedAreaController","Exception in calc_coortopix",e,1);
					}
				//}
			}
		}
	...
	public void updateShape(ShapeStruct s) { //override
		m_shape = s;
		if(getAreaCtrl()!=null && m_shape==null)
		{
			getAreaCtrl().resetEditShape();
		}
	}
	...
	public void gotoMap() {
		...
		if (checkObject(object, AreaVO.class)) {
			...
			try {
				...
				log.debug(e.getMessage());
				log.warn(e.getMessage(), e);
				Error.getError().addError("PredefinedAreaController","Exception in gotoMap",e,1);
			}
		}
	}
