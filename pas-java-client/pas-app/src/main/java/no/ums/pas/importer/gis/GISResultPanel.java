...
public class GISResultPanel extends PreviewList { //SearchPanelResults {
    ...
    public void fill(GISList gislist,final String import_type, boolean b_run_hittest) {
        m_gislist = gislist;
        //m_b_run_hittest = b_run_hittest;
        SwingUtilities.invokeLater(new Runnable(){
            public void run()
            {
                start_search(import_type);
    ...
    public void start_search(String import_type) {
            ...
            if(get_gislist().size()<=n_threshold)
            {
                ...
                for(int h=0; h < get_gislist().size(); h++) {
                    ...
                    for(int i=0; i < house.get_inhabitantcount(); i++) {
                        ...
                        {
                            Object [] line  ;
                            if("Street".equals(import_type)){
                                line = new Object[] {n_recordcount, house.get_municipal(), house.get_streetid(), house.get_houseno(),
                            ...
                            else if("StreetApartment".equals(import_type)){
                                line = new Object[] {n_recordcount,((GISRecordApartmentID)house).get_municipal(), ((GISRecordApartmentID)house).get_streetid(), ((GISRecordApartmentID)house).get_houseno(),
                                        ((GISRecordApartmentID)house).get_letter(), ((GISRecordApartmentID)house).get_name1(), ((GISRecordApartmentID)house).get_name2(), inhabitant, inhabitant.get_number(), inhabitant.get_mobile(),
                                        inhabitant.get_postno(), inhabitant.get_postarea(), b_include, hit, inhabitant.get_lon(), inhabitant.get_lat()};
                            }
                            else{
                                line = new Object[] {n_recordcount, ((GISRecordProperty)house).get_municipal(), ((GISRecordProperty)house).getM_sz_gnr(), ((GISRecordProperty)house).getM_sz_bnr(),
                                        ((GISRecordProperty)house).getM_sz_fnr(), ((GISRecordProperty)house).get_name1(), ((GISRecordProperty)house).get_name2(), inhabitant, inhabitant.get_number(), inhabitant.get_mobile(),
                                        inhabitant.get_postno(), inhabitant.get_postarea(),b_include,((GISRecordProperty)house).getM_sz_snr(), hit, inhabitant.get_lon(), inhabitant.get_lat()};
