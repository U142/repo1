    ...
    public class UMSAlertiXParser
    {
        ...
        public void CheckFiles()
        {
            while (oController.running)
            {
                ...
                {
                    // copy files
                    string[] copyFiles = Directory.GetFiles(oController.CopyFrom, "LBA_*.xml");
                    Array.Sort(copyFiles);
                    foreach (string fileName in copyFiles)
                    {
                        if (!oController.running) break;
                        foreach (string path in oController.CopyTo)
                        {
                            FileInfo fileinfo = new FileInfo(fileName);
                            File.Copy(fileName, path + "\\" + fileinfo.Name + ".tmp");
                            File.Move(path + "\\" + fileinfo.Name + ".tmp", path + "\\" + fileinfo.Name);
                        }
