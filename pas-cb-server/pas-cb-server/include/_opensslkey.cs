...
namespace JavaScience
{
    ...
    public class opensslkey
    {
        ...
        public static string DecodePEMKey(String pemstr, String password)
        {
            //String pemstr = File.ReadAllText(filename).Trim();
            ...
            else if (pemstr.StartsWith(pemp8encheader) && pemstr.EndsWith(pemp8encfooter))
            {
                ...
                {
                    RSACryptoServiceProvider rsa = DecodeEncryptedPrivateKeyInfo(pkcs8encprivatekey, password);
                    if (rsa != null)
                    {
                        ret = rsa.ToXmlString(true);
                    }
        ...
        public static byte[] DecryptPBDK2(byte[] edata, byte[] salt, byte[] IV, String password, int iterations)
        {
        ...
        public static byte[] DecodeOpenSSLPrivateKey(String instr, String password)
        {
