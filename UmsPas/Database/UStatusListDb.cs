	...
	sprintf(szSQL, "SELECT isnull(head.l_type,0) l_sendingtype, isnull(head.l_items, -1) l_totitem, "
        ...
        public UStatusListResults GetStatusList(ref ULOGONINFO logon)
        {
            ...
            String szSQL = String.Format(
                ...
                "BBDEPARTMENT dept, BBPROJECT_X_REFNO projx, BBPROJECT proj WHERE dept.l_comppk={0} AND info.l_type=4 AND info.l_refno=projx.l_refno AND projx.l_projectpk=proj.l_projectpk AND info.l_deptpk=dept.l_deptpk AND info.l_refno*=head.l_refno " +
                "AND info.l_group in (2,3,4,8,9) GROUP BY info.l_deptpk, dept.l_deptpk, dept.sz_deptid, proj.l_projectpk, projx.l_projectpk, projx.l_refno, info.l_refno, head.l_status " +
